      ...
      <div className="container mx-auto px-4 py-16">
        ...
        >
          {faqs.map((faq) => (
            <motion.div
              ...
              className="mb-4 bg-primary-deep rounded-lg shadow-lg overflow-hidden border border-secondary-gold/20"
            >
              <button
                onClick={() => setExpandedId(expandedId === faq.id ? null : faq.id)}
                className={`w-full px-6 py-4 text-left flex items-center justify-between transition-colors duration-200 ${expandedId === faq.id ? 'bg-secondary-gold/10 text-secondary-gold' : 'hover:bg-primary-charcoal text-accent-ivory'
                  }`}
              >
                <h3 className="text-lg font-bold pr-4 font-serif">{faq.question}</h3>
                <motion.span
                  animate={{ rotate: expandedId === faq.id ? 180 : 0 }}
                  transition={{ duration: 0.3 }}
                  className="text-2xl text-secondary-gold flex-shrink-0"
                >
                  ▼
                </motion.span>
              </button>
        ...
        >
          ...
          <motion.a
            ...
            whileTap={{ scale: 0.95 }}
            href="/contact"
            className="inline-block bg-secondary-gold hover:bg-secondary-warm text-primary-deep font-bold py-3 px-8 rounded-full transition duration-300"
          >
            Contact Us
          </motion.a>
        </motion.div>
